// DOM Elements
        ...
        // Audio
        // Placeholder URLs for sounds. Replace with actual direct links or base64 encoded audio.
        // Example Google Drive direct link: https://docs.google.com/uc?export=download&id=YOUR_FILE_ID
        // For actual deployment, consider hosting these files or embedding small ones as base64.
                ...
                for (let col = 0; col < 8; col++) {
                    ...
                    if (selectedPiece) {
                        ...
                        if (!playerMustCapture(currentPlayer)) {
                            const normalMoves = getSimpleMoves(selectedPiece.row, selectedPiece.col); // Reusing AI's simple moves for player
        ...
        function saveHistory() {
            ...
            if (
                moveHistory.length > 30
            ) {
                moveHistory.shift(); // Keep history size manageable
            }
        }
        ...
        function attemptMove(fromRow, fromCol, toRow, toCol) {
            ...
            if (captureMove) {
                ...
                playCaptureSound(captureMove.captureRow, captureMove.captureCol); // Play capture sound and particles
                ...
                if (nextCaptures.length) {
                    ...
                    checkVictory();
                    saveGame();
                    return; // Player must make another capture
                }
            ...
            if (mustCapture) {
                return; // Player must capture but tried a regular move
            }
            ...
            if (piece.king) {
                ...
                if (rowDiff !== colDiff) {
                    return; // King must move diagonally
                }
                // Check if path is clear for king
                ...
                if (toRow !== fromRow + dir) {
                    return; // Regular piece can only move forward
                }
                if (Math.abs(toCol - fromCol) !== 1) {
                    return; // Regular piece can only move one step diagonally
                }
            }
            ...
            board[toRow][toCol] = piece;
            board[fromRow][fromCol] = null;
            playMoveSound(); // Play regular move sound
            promoteIfNeeded(piece, toRow);
            selectedPiece = null;
            switchTurn();
            checkVictory();
                ...
                if (piece.player === currentPlayer) {
                    ...
                    if (mustCapture) {
                        if (!getCaptures(row, col).length) {
                            // If there are mandatory captures, but this piece cannot capture, prevent selection
                            return;
                        }
                    }
        ...
        function updateScore() {
            ...
            localStorage.setItem(
                SCORES_STORAGE_KEY,
                JSON.stringify({ player: playerScore, ai: aiScore })
            );
        }
        ...
        undoBtn.addEventListener("click", () => {
            ...
            selectedPiece = null;
            currentPlayer = "player"; // Always revert to player's turn after undo
        ...
        function loadGame() {
            ...
            moveHistory = data.moveHistory || []; // Ensure moveHistory is loaded
            updateScore();
        ...
        function getSimpleMoves(row, col) {
            ...
            if (piece.king) {
                ...
                const dir = piece.player === "player" ? -1 : 1; // Player moves up (-1), AI moves down (+1)
                for (const dc of [-1, 1]) {
                    const r = row + dir;
                    const c = col + dc;
        ...
        function getAllPossibleMoves() {
            ...
            if (moves.length) {
                return moves; // If captures are available, AI must capture
            }
        ...
        function mediumAI() {
            ...
            // If no captures, play a random normal move
            const normalMoves = moves.filter(m => m.type === "move");
            if (normalMoves.length) {
                executeAIMove(
                    normalMoves[
                        Math.floor(Math.random() * normalMoves.length)
                    ]
                );
            } else {
                // Fallback to easyAI if no normal moves (shouldn't happen if getAllPossibleMoves is correct)
                easyAI();
            }
        }
        ...
        function evaluateBoard() {
            ...
            for (let r = 0; r < 8; r++) {
                for (let c = 0; c < 8; c++) {
                    ...
                    let value = piece.king ? 5 : 1; // King is worth more
                    // Add positional advantage (e.g., pieces closer to promotion, center control)
                    if (!piece.king) {
                        if (piece.player === "ai") {
                            value += r * 0.1; // AI pieces moving forward are better
                        } else {
                            value += (7 - r) * 0.1; // Player pieces moving forward are better
                        }
                    }
        ...
        function hardAI() {
            ...
            let bestMove = null;
            let bestScore = -Infinity; // AI wants to maximize its score

            for (const move of moves) {
                const clone = JSON.parse(JSON.stringify(board)); // Deep clone the board
                simulateMove(move);
                const score = evaluateBoard();
                board = JSON.parse(JSON.stringify(clone)); // Restore the board
        ...
        function simulateMove(move) {
            ...
            promoteIfNeeded(piece, move.toRow); // Simulate promotion
        }

        function executeAIMove(move) {
            setTimeout(() => {
                saveHistory(); // Save board state before AI move
                const piece = board[move.fromRow][move.fromCol];
                board[move.toRow][move.toCol] = piece;
                board[move.fromRow][move.fromCol] = null;
                ...
                // Check for multiple captures for AI
                ...
                if (nextCaptures.length && move.type === "capture") {
                    // AI must make another capture
                    statusText.textContent = "IA fazendo múltiplas capturas...";
                    // Find the best next capture for AI
                    ...
                    if (bestNextCapture) {
                        executeAIMove(bestNextCapture); // Recursively call for next capture
                        return;
                    }
                }

                selectedPiece = null; // Clear selected piece after AI turn
                currentPlayer = "player";
                statusText.textContent = "Sua vez";
                if (checkVictory()) {
                    // Victory handled by checkVictory
                } else {
                    saveGame();
                    renderBoard();
                }
        ...
        function switchTurn() {
            ...
            setTimeout(() => {
                if (checkVictory()) return; // Check victory again before AI moves
        ...
        // --- Part 5: Visuals, Audio, Optimizations ---

        function explodeParticles(row, col) {
            if (!isSoundOn) return; // Only trigger particles if sounds are on
            ...
            for (let i = 0; i < 20; i++) {
                ...
                particle.style.animationDuration = `${Math.random() * 0.5 + 0.5}s`; // 0.5-1s
